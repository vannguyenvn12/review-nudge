...
export async function logJobAction(
  ...
  const completed_at_raw = (formData.get("completed_at") as string | null)?.trim();
  ...
  const { data: profile, error: profileError } = await (supabase.from("profiles") as any)
    .select("delay_hours")
    .eq("id", user.id)
    .single() as { data: { delay_hours: number } | null; error: { message: string } | null };
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const { error: rrError } = await (supabase.from("review_requests") as any).insert({
    job_id: jobData.id,
    status: "pending",
    scheduled_at,
  });
